...
class Confirmation(Resource):
    @classmethod
    def get(cls, confirmation_id: str):  # es get porque regresamos una pagina
...
class ConfirmationByUser(Resource):
    ...
    def get(cls, user_id: int):
        # returns confirmations for a given user. use for testing or admins idk
        user = UserModel.find_by_id(user_id)
        if not user:
            return {"message": gettext("user_not_found")}, 404

        return ({"current_time": int(time()),
                 "confirmation": [confirmation_schema.dump(each) for each in
                                  user.confirmation.order_by(
                                      db.desc(ConfirmationModel.expire_at))], }, 200,)
